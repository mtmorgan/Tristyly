...
Change in morph frequence can be followed in several ways.

The `iterate()` function takes an initial population and iterates it
until a specific number of generations has passed. `plot_iterations()`
visualizes the result.

```{r iterate}
its <- iterate(isoplethy(50), 1000)
its
tail(its)
plot_iterations(its)
```

The `iterate_to_*()` functions repeatedly iterate a single population
until loss of one or two morphs.
